...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const doc = new Document({
    sections: [{
      children: [
        ...
        new Paragraph({ text: "Client Onboarding Form — FLW-OPS-001", heading: HeadingLevel.HEADING_1, alignment: AlignmentType.CENTER }),
        new Paragraph({ text: "" }),
        new Paragraph({ text: "SECTION A — CLIENT DETAILS", heading: HeadingLevel.HEADING_2 }),
        new Table({
        ...
        new Paragraph({ text: "" }),
        new Paragraph({ text: "SECTION B — REQUIREMENTS", heading: HeadingLevel.HEADING_2 }),
        new Table({
          width: { size: 100, type: WidthType.PERCENTAGE },
          rows: [
            row("Current WhatsApp Use", s.current_whatsapp_use ?? ""),
            row("Existing WABA", s.existing_waba === true ? "Yes" : s.existing_waba === false ? "No" : "Unsure"),
        ...
        new Paragraph({ text: "" }),
        new Paragraph({ text: "SECTION C — SIGNATURES", heading: HeadingLevel.HEADING_2 }),
        new Table({
          width: { size: 100, type: WidthType.PERCENTAGE },
          rows: [
  ...
  const filename = `Flowa - ${s.company_legal_name ?? id} - Onboarding (FLW-OPS-001).docx`
